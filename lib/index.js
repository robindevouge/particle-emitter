...
const findMoveVector = (vect, distance, angle) => {
...
class Particle {
    ...
    startCoord;
    endCoord;
    constructor({ scale = 1, lifetime = 1, mainClass = 'particle', randomClasses = null, gsapTweenOptions = {}, destroyDuration = 0, onSpawn = () => { }, onDestroy = () => { }, onTweenComplete = () => { }, emitter }) {
    ...
    spawn = () => {
        ...
        gsap_1.default.set(this.element, {
            autoAlpha: 0,
            x: this.startCoord.x,
            y: this.startCoord.y,
            scale: this.scale,
    ...
    move = () => {
        ...
        gsap_1.default.set(this.element, { autoAlpha: 1 });
        // tween
        this.tween = gsap_1.default.to(this.element, Object.assign({
            x: this.endCoord.x,
            y: this.endCoord.y,
    ...
    destroy = (duration = this.destroyDuration) => {
        gsap_1.default.to(this.element, {
            autoAlpha: 0,
            duration: duration,
            onComplete: () => {
                this.element.remove();
...
class Emitter {
    ...
    start = (callback = () => { }) => {
        if (this.spawnLoop)
            return;
        this.spawnLoop = gsap_1.default.set(this.spawnParticle, {
            onRepeat: () => {
                this.spawnParticle();
            },
